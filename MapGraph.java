...
public class MapGraph {
	 ...
	 * 	of node
	 */
	private HashMap<MapNode,Double> getNeighbors(MapNode node) {
			 					     ...
			 					     Consumer<GeographicPoint> nodeSearched)
	{
		...
		while (!toExplore.isEmpty()) {
			...
			HashMap<MapNode,Double> neighbors = getNeighbors(next);
			for (MapNode neighbor : neighbors.keySet()) {
				...
				while (!toExplore.isEmpty()) {
					...
					if(!visited.contains(next)){
						...
						for (MapNode neighbor : neighbors.keySet()) {
							...
							neighbor.setDistanceToEnd(goal);
							
							if ((neighbors.get(neighbor)+next.getDistanceFromStart())< neighbor.getDistanceFromStart()){
								
								//parentMap.remove(neighbor);
								neighbor.setDistanceFromStart(next.getDistanceFromStart() + neighbors.get(neighbor));
								parentMap.put(neighbor, next);
								toExplore.add(neighbor);
							}
						}	
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		startNode.setDistanceFromStart(0.0);
		startNode.setDistanceToEnd(goal);
		toExplore.add(startNode);
		MapNode next = null;
		...
		while (!toExplore.isEmpty()) {
			...
			//next.setDistanceToEnd(next.getLocation().distance(goal));
			next.setDistanceToEnd(goal);			
			 // hook for visualization
			nodeSearched.accept(next.getLocation());
			...
			if(!visited.contains(next)){
				...
				for (MapNode neighbor : neighbors.keySet()) {
					...
					if ((neighbors.get(neighbor) + next.getDistanceFromStart())< neighbor.getDistanceFromStart()){
						...
						//neighbor.setDistanceToEnd(next.getDistanceFromStart() + neighbor.getLocation().distance(goal));
						neighbor.setDistanceFromStart(next.getDistanceFromStart() + neighbors.get(neighbor));
